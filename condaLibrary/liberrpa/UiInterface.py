...
from liberrpa.UI._UiAutomation import get_child_controls
...
def _get_parent(
    ...
    if uiTarget is None:
        raise UiOperationError("Expected a UIA control while getting its parent.")

    controlRoot = uiautomation.GetRootControl()
    for intCurrentLevel in range(1, upwardLevel + 1):
        controlParent = uiTarget.GetParentControl()
        if controlParent is None or uiautomation.ControlsAreSame(controlParent, controlRoot):
            raise UiElementNotFoundError(f"Not found its parent element at upwardLevel {intCurrentLevel}.")

        uiTarget = controlParent

    selectorParentUia = _UiElement.get_control_selector(control=uiTarget)
    Log.debug(f"Found parent at upwardLevel {upwardLevel}: {selectorParentUia}")
    delay(postDelay)
...
def _get_children(
    ...
    if selector.get("category") == "html":
        ...
        selectorWindow: SelectorWindow = ensure_selector_window(_UiElement.get_control_selector(control=controlTop))
    ...
    uiTarget, _ = _UiElement.get_element_with_pre_delay(selector=selector, preDelay=preDelay)
    if uiTarget is None:
        raise UiOperationError("Expected a UIA control while getting its children.")

    listControlChildren = get_child_controls(control=uiTarget)
...
if __name__ == "__main__":
    ...
    listTemp = get_child_controls(control=uiTarget)

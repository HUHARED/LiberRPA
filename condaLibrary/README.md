...
`liberrpa` is the Python core package of LiberRPA.

It provides the public APIs used by RPA projects and also includes internal modules used by LiberRPA tools, such as initialization utilities, Chrome extension communication, and the local Flask server.

In normal use, you do not need to install this package separately. The recommended way to use LiberRPA is to download the complete LiberRPA release package, which is designed to include the required Python environment, editor, project templates, desktop tools, local server, Chrome extension support, and external tools such as FFmpeg.

The `liberrpa` package is also published on PyPI for advanced scenarios, such as development, debugging, manually preparing a compatible environment, or following official LiberRPA update instructions.

Installing the Python package alone is not enough to set up a complete LiberRPA runtime. Advanced users who install `liberrpa` manually are responsible for preparing all required runtime dependencies and ensuring that the environment is compatible with LiberRPA.
...
This section contains the code snippets you can use.
...
- [API](#api)
  ...
  - [System](#system)
    ...
    - [get\_computer\_info](#get_computer_info)
    - [exit](#exit)
...
Get elapsed seconds since the current RPA project started.

### PrjArgs.errorObj

Get the current flow error object, or None if no error is being handled.
...
Create a basic LiberRPA Python file template with a managed import block, main(), and a direct-run entry point.

### def a function

Define a function with parameters and a return annotation.
...
Insert a file name comment.

### # type: ignore

Add a # type: ignore comment at the end of the current line to suppress a type checker warning, with a reason.
...
Run a block when the previous if/elif conditions are false.

### elif

Add another condition after an if block.

### match case

Use Python structural pattern matching to choose a branch by value or pattern.

### case

Add a case branch inside a match block.

### for-index, item

Loop over items with enumerate() and a zero-based or custom starting index.

### for-item

Loop over each item in an iterable object.

### for-counting cycle

Loop over a sequence of numbers generated by range().
...
Handle exceptions with try/except, and optionally run else/finally blocks.

### retry

Retry a block until it succeeds, or raise the last error after the maximum try count is reached.
...
clickMode: Click mode, one of ["single_click", "double_click", "down", "up"].
executionMode: Execution mode, either "simulate" or "api". "simulate" will move the cursor, support all arguments of the function. "api" will not move the cursor, it can handle some situations that the target element be covered, but it supports less arguments than "simulate".
position: Target position, one of ["center", "top_left", "top_right", "bottom_left", "bottom_right"]. It will only work if executionMode is "simulate".
...
```python
@overload
def get_parent(selector: liberrpa.UI._UiDict.SelectorHtml, upwardLevel: int = 1, timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> liberrpa.UI._UiDict.SelectorHtml: ...
@overload
def get_parent(selector: liberrpa.UI._UiDict.SelectorUia, upwardLevel: int = 1, timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> liberrpa.UI._UiDict.SelectorWindow | liberrpa.UI._UiDict.SelectorUia: ...
```
...
```text
selector: A selector dictionary generated by UI Analyzer for locating an element. You can edit it to make it more concise or more robust in different situations.
upwardLevel: Number of direct parent levels to move upward.
timeout: Maximum time allowed for normal completion, in milliseconds. Values below 3000 are treated as 3000. The actual elapsed time may be longer if LiberRPA enters its hard-timeout fallback before raising a timeout-related exception.
preDelay: Time to wait before performing the action, in milliseconds.
postDelay: Time to wait after performing the action, in milliseconds.
```
...
For both UIA and HTML elements, this function returns selectors of direct
child elements. UIA children whose Name is empty are included.

Overload signatures:

```python
@overload
def get_children(selector: liberrpa.UI._UiDict.SelectorHtml, timeout: int = 30000, preDelay: int = 300, postDelay: int = 200) -> list[liberrpa.UI._UiDict.SelectorHtml]: ...
@overload
def get_children(selector: liberrpa.UI._UiDict.SelectorWindow | liberrpa.UI._UiDict.SelectorUia, timeout: int = 30000, preDelay: int = 300, postDelay: int = 200) -> list[liberrpa.UI._UiDict.SelectorUia]: ...
```
...
```text
list[SelectorUia] | list[SelectorHtml]: Selectors of direct child elements.
```
...
```python
@overload
def get_attr_dictionary(selector: liberrpa.UI._UiDict.SelectorWindow | liberrpa.UI._UiDict.SelectorUia, timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> liberrpa.UI._UiDict.DictUiaAttr: ...
@overload
def get_attr_dictionary(selector: liberrpa.UI._UiDict.SelectorHtml, timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> liberrpa.UI._UiDict.DictHtmlAttr: ...
@overload
def get_attr_dictionary(selector: liberrpa.UI._UiDict.SelectorImage, timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> liberrpa.UI._UiDict.DictImageAttr: ...
```
...
```python
@overload
def get_selection(selector: liberrpa.UI._UiDict.SelectorHtml, selectionType: Literal['text'] = 'text', timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> str: ...
@overload
def get_selection(selector: liberrpa.UI._UiDict.SelectorHtml, selectionType: Literal['value'], timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> str: ...
@overload
def get_selection(selector: liberrpa.UI._UiDict.SelectorHtml, selectionType: Literal['index'], timeout: int = 10000, preDelay: int = 300, postDelay: int = 200) -> int: ...
```
...
```python
@overload
def execute_js_code(browserObj: liberrpa.Browser.BrowserObj, jsCode: str, returnImmediately: Literal[True]) -> None: ...
@overload
def execute_js_code(browserObj: liberrpa.Browser.BrowserObj, jsCode: str, returnImmediately: Literal[False] = False) -> JsonValue: ...
```
...
```python
@overload
def read_cell(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, cell: ExcelCell, returnDisplayed: Literal[True] = True) -> str: ...
@overload
def read_cell(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, cell: ExcelCell, returnDisplayed: Literal[False]) -> ExcelCellValue: ...
```
...
```python
@overload
def read_row(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, startCell: ExcelCell, returnDisplayed: Literal[True] = True) -> list[str]: ...
@overload
def read_row(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, startCell: ExcelCell, returnDisplayed: Literal[False]) -> list[ExcelCellValue]: ...
```
...
```python
@overload
def read_column(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, startCell: ExcelCell, returnDisplayed: Literal[True] = True) -> list[str]: ...
@overload
def read_column(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, startCell: ExcelCell, returnDisplayed: Literal[False]) -> list[ExcelCellValue]: ...
```
...
```python
@overload
def read_range_list(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, startCell: ExcelCell, endCell: ExcelCell | None = None, *, returnDisplayed: Literal[True] = True) -> list[list[str]]: ...
@overload
def read_range_list(excelObj: liberrpa.Common._Excel.ExcelObj, sheet: ExcelSheet, startCell: ExcelCell, endCell: ExcelCell | None = None, *, returnDisplayed: Literal[False]) -> list[list[ExcelCellValue]]: ...
```
...
Create a DatabaseConnection context manager. See liberrpa.Database.DatabaseConnection's docstring for details.
...
```python
@overload
def fetch_one(connObj: sqlalchemy.engine.base.Connection, query: str, params: dict[str, typing.Any] | None = None, *, returnDict: Literal[True] = True) -> dict[str, typing.Any] | None: ...
@overload
def fetch_one(connObj: sqlalchemy.engine.base.Connection, query: str, params: dict[str, typing.Any] | None = None, *, returnDict: Literal[False]) -> list[typing.Any] | None: ...
```
...
```python
@overload
def fetch_all(connObj: sqlalchemy.engine.base.Connection, query: str, params: dict[str, typing.Any] | None = None, *, returnDict: Literal[True] = True) -> list[dict[str, typing.Any]]: ...
@overload
def fetch_all(connObj: sqlalchemy.engine.base.Connection, query: str, params: dict[str, typing.Any] | None = None, *, returnDict: Literal[False]) -> list[list[typing.Any]]: ...
```
...
```python
@overload
def get(dictObj: dict[K, V], key: K) -> Optional[V]: ...
@overload
def get(dictObj: dict[K, V], key: K, default: D) -> Union[V, D]: ...
```
...
```python
@overload
def pop(dictObj: dict[K, V], key: K) -> Optional[V]: ...
@overload
def pop(dictObj: dict[K, V], key: K, default: D) -> Union[V, D]: ...
```
...
Create an ftputil.FTPHost context manager. See ftputil.FTPHost documentation for details.
...
```python
@overload
def show_message_box(title: str, message: str, *, type: Literal['info'] = 'info', infoButton: Literal['ok'] = 'ok') -> Literal['ok']: ...
@overload
def show_message_box(title: str, message: str, *, type: Literal['info'] = 'info', infoButton: Literal['okcancel', 'yesno', 'retrycancel']) -> bool: ...
@overload
def show_message_box(title: str, message: str, *, type: Literal['info'] = 'info', infoButton: InfoButton) -> Union[Literal['ok'], bool]: ...
@overload
def show_message_box(title: str, message: str, *, type: Literal['warning', 'error']) -> Literal['ok']: ...
@overload
def show_message_box(title: str, message: str, *, type: Literal['question']) -> Literal['yes', 'no']: ...
```
...
```python
@overload
def mouse_trigger(func: collections.abc.Callable[..., T], args: list[typing.Any] | None = None, button: MouseButton = 'left', pressCtrl: bool = False, pressShift: bool = False, pressAlt: bool = False, pressWin: bool = False, timing: Literal['on_press', 'on_release'] = 'on_release', notify: bool = True, *, block: Literal[False]) -> None: ...
@overload
def mouse_trigger(func: collections.abc.Callable[..., T], args: list[typing.Any] | None = None, button: MouseButton = 'left', pressCtrl: bool = False, pressShift: bool = False, pressAlt: bool = False, pressWin: bool = False, timing: Literal['on_press', 'on_release'] = 'on_release', notify: bool = True, *, block: Literal[True] = True) -> T: ...
```

...
* **Security**
  ...
  * LiberRPA can run offline and does not actively collect any of your information.
    ...
    * Please note that LiberRPA relies on many third-party open-source dependencies from ecosystems such as Python, Node.js, PyPI, conda-forge, and npm. While LiberRPA tries to use widely adopted and actively maintained dependencies, it cannot independently guarantee the security or long-term stability of every upstream dependency. For more details, please see [Dependencies and Acknowledgments](#dependencies-and-acknowledgments).
...
# [🚧](https://emojidb.org/limit-emojis) Limitations
...
LiberRPA is designed to support portable and offline use, but the portable environment must be initialized on an online computer first.

During the first initialization, open the bundled Editor based on VS Code and allow it to complete its setup process, including downloading and installing the required VS Code extensions. This step is not fully bundled in the release package because the extension data can be large and may contain machine-specific configuration generated by VS Code.

After the Editor and extensions have been initialized successfully, you can copy the initialized LiberRPA folder to another Windows computer for offline use.

Recommended process:

1. Extract the complete LiberRPA release package on an online Windows computer.
2. Run the LiberRPA initialization program.
3. Open the bundled Editor and wait until the required extensions are installed.
4. Close the Editor.
5. Copy the initialized LiberRPA folder to the offline computer.
6. Run LiberRPA from the copied folder.

If the Editor has never been opened and initialized on an online computer, some VS Code extensions may be missing in the offline environment.

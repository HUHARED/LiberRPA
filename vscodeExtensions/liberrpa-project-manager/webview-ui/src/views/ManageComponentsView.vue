  ...
  <v-card class="mx-auto" max-width="1180">
    ...
    <v-card-text class="pa-5">
      <v-row class="mb-1">
        ...
        <v-col cols="12" sm="6" lg="3">
          <v-card variant="tonal" class="pa-3 h-100">
            ...
            <v-chip
              size="small"
              :color="getStateColor(projectState.componentsState)">
        ...
        <v-col cols="12" sm="6" lg="3">
          <v-card variant="tonal" class="pa-3 h-100">
            ...
            <v-chip
              size="small"
              :color="getStateColor(projectState.environmentState)">
      ...
      <v-card variant="outlined" class="mb-5">
        ...
        <v-card-text
          v-if="directDependencyRows.length === 0"
...
const repositoryCatalog = computed(() => {
  if (componentManagementStore.repositoryCatalog === null) {
    throw new Error("Manage Components Repository catalog is unavailable.");
  }
  return componentManagementStore.repositoryCatalog;
});
  ...
  Object.entries(projectState.value.manifest.componentDependencies)
    .map(([componentId, currentRequirement]) => {
      const lockedComponent =
        projectState.value.componentsLock?.components[componentId];
      return {
        componentId,
        packageName:
...
function repair(): void {
  if (
    projectManagerStore.busy ||
    projectState.value.repairState !== "available"
  ) {
    return;
  }
